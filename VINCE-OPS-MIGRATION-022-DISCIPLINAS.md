# VINCE Ops — Migration 022 + Prompt 3D (Disciplinas)
> Executar a migration AGORA (antes do Prompt 3B — ela reescreve RPCs de categoria).
> O Prompt 3D vai pro Cursor DEPOIS do 3C validado.

---

## MIGRATION 022 — Executar no Supabase
Salvar também como `supabase/migrations/022_unicidade_travas_disciplinas.sql`
...
```sql
select codigo, nome, cor_bg, cor_texto from disciplinas_config order by ordem;
```

---

## PROMPT 3D — Disciplinas dinâmicas (mandar pro Cursor APÓS validar 3C)

```
Migration 022 executada. Existe agora a tabela
disciplinas_config (codigo imutável, nome editável,
cor_bg, cor_texto, ordem, sistema, ativo) com seed
HID/PPCI/SPK, e as RPCs upsert_disciplina_config,
delete_disciplina_config e copiar_estrutura_disciplina.

As travas de fases obrigatórias foram removidas do banco.

## Etapa 3D: disciplinas dinâmicas em todo o sistema.

### 1. src/lib/disciplinaConfig.ts + contexto

- Mesmo padrão do faseConfig: fetchDisciplinasConfig()
  com cache, invalidateDisciplinasCache(), tipos
- DisciplinasConfigContext no App.tsx (junto ao de fases)
- No boot e ao invalidar: injetar CSS variables dinâmicas
  para cada disciplina:
  document.documentElement.style.setProperty(
    `--disc-${codigo.toLowerCase()}-bg`, cor_bg)
  document.documentElement.style.setProperty(
    `--disc-${codigo.toLowerCase()}-text`, cor_texto)

### 2. Substituir listas hardcoded HID/PPCI/SPK

Buscar em todo src/ e substituir por dados do contexto:
- DisciplinaTabs (Configurações, filtros do Histórico)
- Criação de projeto (chips de seleção)
- "+ Disciplina" na Home do projeto
- Badges de disciplina em todo lugar: discToneClasses
  vira função que resolve cores de disciplinas_config —
  para HID/PPCI/SPK pode manter as classes CSS atuais,
  para custom usa as CSS vars dinâmicas via style inline
- Sidebar do projeto (tabs de disciplina)
- Gráficos e tooltips ("HID: Xh | PPCI: Yh" →
  montar dinamicamente a partir das disciplinas do projeto)

Nome exibido SEMPRE de disciplinas_config.nome; o codigo
continua sendo a chave interna em projetos.disciplinas,
tarefas.disciplina, fases_config.disciplina etc.
Nada muda no formato dos dados existentes.

### 3. Configurações → seção "Disciplinas"

Nova sub-seção em Templates (primeira tab, antes de
Templates de checklist):
- Lista de disciplinas_config: nome editável inline,
  dois color pickers (fundo e texto) com preview do
  badge ao lado, setinhas ↑↓, toggle ativo
- sistema = true: cadeado (sem excluir), mas nome e
  cores SÃO editáveis
- sistema = false: excluir com ConfirmModal danger →
  delete_disciplina_config (tratar o erro de projetos
  ativos com mensagem amigável)
- Botão "+ Nova disciplina": modal com
  - Nome (obrigatório) → codigo = slug UPPER_SNAKE
  - Cor de fundo + cor de texto (pickers com preview)
  - "Copiar estrutura de:" select opcional
    (fases + categorias + templates) →
    rpc copiar_estrutura_disciplina após criar
  - Toast: "Disciplina criada. X fases, Y categorias
    e Z templates copiados."
- SettingsSaveBar para edições em lote

### 4. Ajustes decorrentes das travas removidas

Em Configurações → Fases:
- Remover tooltip/lock de "fase obrigatória" — todas
  as fases têm toggle e podem ser excluídas
- Tratar os erros das RPCs com mensagens amigáveis:
  "Há projetos atualmente nesta fase..." e
  "A disciplina precisa de ao menos uma fase ativa."
  (exibir como toast de erro, não crash)

### 5. Limitação conhecida (registrar, não resolver)

horas_estimadas_hid / horas_estimadas_ppci são colunas
fixas — disciplinas novas ficam sem campo de estimativa
por enquanto. Registrar no DECISIONS.md:
"Estimativas por disciplina custom virão em campo jsonb
em iteração futura; colunas hid/ppci mantidas por
compatibilidade."

npm run build deve passar.
Validar: renomear PPCI e mudar sua cor → reflete em
badges, tabs e gráficos; criar disciplina "Elétrico"
copiando estrutura do HID → aparece na criação de
projeto com fases e templates; excluir disciplina em
uso → erro amigável.
```
